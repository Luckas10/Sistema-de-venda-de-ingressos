...
def criar_tabela(table_name, name_db):
    ...
    cursor.execute(f"CREATE TABLE {table_name} (ticket text)")
...
def criar_tabela_user(table_name, name_db):
    ...
    cursor.execute(f"CREATE TABLE {table_name} (filme text, ticket text, sala text, horario text)")
...
def deletar_compra(ticket_informado, user):
    try:
        ...
        cursor_admin.execute("DELETE FROM Tickets WHERE ticket = ?", (str(ticket_informado),))
        cursor_users.execute(f"DELETE FROM {user} WHERE ticket = ?", (str(ticket_informado),))
